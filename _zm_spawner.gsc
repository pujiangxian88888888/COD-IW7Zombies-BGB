...
function init()
{
	...
	// added functionality to respawn elemental type zombies cleaned up.
...
function player_attacks_enemy( player, amount, type, point, weapon, direction_vec, tagName, modelName, partName, dFlags, inflictor, chargeLevel )
{
	...
	if ( IS_TRUE( player.allow_zombie_to_target_ai) || !player util::is_ads() )
	{
...
function enemy_death_detection()
{
	...
	for ( ;; )
	{
		...
		if ( !player_attacker( attacker ) && !IS_TRUE( attacker.allow_zombie_to_target_ai) )
		{
			continue;
		}
...
function zombie_damage_failsafe()
{
	...
	while (1)
	{
		...
		if (self istouching(self.enemy))
		{
			...
			if (!isdefined(self.enemy) || !IsPlayer( self.enemy ) || self.enemy hasperk( PERK_JUGGERNOG ) )
			{
				continue;
			}
...
function deregister_zombie_damage_callback( func )
{
...
function zombie_death_event( zombie )
{
	...
	{
		// If a zombie dies with the kvp (script_parameters), notify the attacker. script_parameter is inherited from the spawn location.
		if( isdefined(zombie.script_parameters) )
		{
			attacker notify( "zombie_death_params", zombie.script_parameters, IS_TRUE(zombie.completed_emerging_into_playable_area) );
		}

		// Widows wine kill notify
		...
		// If zombie is crawler notify the player of ther crawler kill
		if ( isDefined( zombie ) && IS_TRUE( zombie.missingLegs ) )
		{
	...
	if( IsActor( zombie ) )
	{
		if( IS_TRUE(zombie.damageweapon.doannihilate) )
		{
			zombie zombie_explodes_intopieces( false );
		}
		else
		{
		...
		if ( zombie.damagemod === "MOD_GRENADE" || zombie.damagemod === "MOD_GRENADE_SPLASH" )
		{
			if ( isdefined( attacker ) && isalive( attacker )  &&isPlayer( attacker ) )
			{
				...
				attacker.grenade_multikill_count++; // Black Ops 3 - TU1 Fix; track multikill count in addition to multiattack, for the Five-for-One Challenge
			}
		}
	}
...
function zombie_follow_enemy()
{
	...
	while( 1 )
	{
		...
		else if( IsDefined( self.favoriteenemy ) )
		{
			...
			else if( IS_TRUE(self.is_rat_test) )
			{
			}

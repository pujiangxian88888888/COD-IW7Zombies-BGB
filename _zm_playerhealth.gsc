...
function playerHealthRegen()
{
	...
	for( ;; )
	{
		...
		{
			if( self bgb::is_enabled( "zm_bgb_always_done_swiftly" ) )
			{
				health_regen_delay = 1000;
				health_regen_delay_veryhurt = 1000;
			}
			else
			{
				health_regen_delay = level.playerHealth_RegularRegenDelay;
				health_regen_delay_veryhurt = level.longRegenTime;
			}
			if( gettime() - hurttime < health_regen_delay )
			{
				continue;
			}
			...
			if( veryHurt )
			{
				...
				if( gettime() > hurtTime + health_regen_delay_veryhurt )
				{
					newHealth += regenRate;
				}
			}
